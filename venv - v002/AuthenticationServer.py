...
def authenticate_account(username,password_hashed):
    ...
    try:
        ...
        if stored_hash == password_hashed:
            ...
            conn.commit()
            player = c.execute("SELECT * FROM players WHERE uid=?", (record[0],)).fetchone()
            auth_return = "Authenticated",record[0],record[1],token.hexdigest()
            return [pickle.dumps(auth_return), pickle.dumps(player)]
        else:
            return ["Authentication Failure2",]
    except:
        return ["Authentication Failure4", ]
